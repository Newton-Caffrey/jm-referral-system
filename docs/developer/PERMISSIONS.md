...
| `can_manage_referral_meetings` | Meeting **mutation** capability. Same allow/deny roles as Express Interest (includes archived-mutate gate). Assessor / Support Worker denied. Used for create/edit/schedule/complete/cancel (4B.2.2) and **internal attendee** add/edit/remove / completed attendance correction (4B.2.3). Does not control contact/URL visibility. Does not advance pipeline. Attendee membership does **not** grant this capability. |
