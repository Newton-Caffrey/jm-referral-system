...
| `can_manage_package_cost` | Same commercial gate as `can_express_interest` (Admin / Manager / Coordinator / WP admin). Assessor / Support Worker denied mutations. Does **not** require `override_pipeline_stage`. Covers prepare, automated Package Cost email, and Secure Portal/Other record-submit. Phase **4F.1:** sent packages terminal; Assessor may still **view** package panel amounts when the referral is visible (**EXISTING PRODUCT BEHAVIOUR — REQUIRES FUTURE JM CONFIRMATION**). Owner / Champion / Transition lead membership grants no package permission. Focused UAT **PASS** 2026-08-27 (email send **NOT RUN — CODE REVIEWED**). |
| `can_record_la_decision` | Same commercial gate as `can_express_interest`. Covers recording Approved / Declined / Not Proceeding. Does **not** require `override_pipeline_stage`. Phase **4G.1:** record-once terminal after save; Assessor may still **view** the decision panel (including notes) when the referral is visible (**EXISTING PRODUCT BEHAVIOUR — REQUIRES FUTURE JM CONFIRMATION**). Owner / Champion / Transition lead / package preparer membership grants no LA permission. Focused UAT **PASS** 2026-08-27 (Declined / Not Proceeding / status-change email **NOT RUN — CODE REVIEWED**). |
...
| `can_commence_care` | Same commercial gate. Confirm Care Commenced on `transition_planning` when hard prerequisites are met. Does **not** require `override_pipeline_stage` or `jmrs_manage_occupancies`. Phase **4H.1:** record-once terminal; Assessor may **view** transition panel when referral is visible (**EXISTING PRODUCT BEHAVIOUR — REQUIRES FUTURE JM CONFIRMATION**); Support Worker denied; Owner / Champion / Transition Lead membership grants no commence or Place Resident permission. Focused UAT **PASS** 2026-08-27 (Supported Living manually tested; Own Home **NOT RUN — CODE REVIEWED**). |
...
| `can_view_referral_inbox` | Phase **5B.3**. Requires `VIEW_REFERRALS` + commercial/management roles (JM Admin, Referral Manager, Care Coordinator, WP admin / Platform Admin). Assessor / Support Worker denied. Staff Portal Referral Inbox list/detail. Does not invent a new capability. |
| `can_manage_referral_inbox` | Phase **5B.3**. Requires `can_view_referral_inbox` + `EDIT_REFERRALS`. Gates Start Review / Ignore / Duplicate / error recovery POSTs. Actor always current user. |
