...
| `can_manage_referral_meetings` | Meeting **mutation** capability. Same allow/deny roles as Express Interest (includes archived-mutate gate). Assessor / Support Worker denied. Used for meeting write (4B.2.2), internal attendees (4B.2.3), and **external participants** (4B.2.4). Does not control contact/URL visibility. Does not advance pipeline. Attendee/participant membership does **not** grant this capability. |
