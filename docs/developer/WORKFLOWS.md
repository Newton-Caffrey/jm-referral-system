...
**Portal UI (4B.2.1–4B.2.2):** Nested routes under `/referrals/{id}/meetings/` (`PortalRouter` rewrite **1.2.4**). View via `can_view_referral_meetings`; contacts via `can_view_referral_meeting_contacts`; mutations via `can_manage_referral_meetings` + `MeetingLifecyclePolicy` in `ReferralMeetingService`. Write routes: new / edit / schedule / complete / cancel. Completed and cancelled are terminal (no reopen, no hard delete). Past scheduled dates allowed with warning. Purpose/outcome are operational summaries, not clinical assessments. Attendees remain read-only until Phase 4B.2.3; attendance soft warning before complete was code-reviewed in 4B.2.2 UAT but not manually exercised (no attendees). No emails; no workflow-stage changes. Archived referrals remain mutation-read-only. Meetings remain separate from assessment scheduling.
