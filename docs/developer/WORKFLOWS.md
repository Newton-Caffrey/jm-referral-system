...
**Portal UI (4B.2.1–4B.2.4):** Nested routes under `/referrals/{id}/meetings/` (`PortalRouter` rewrite **1.2.6**). View via `can_view_referral_meetings`; contacts via `can_view_referral_meeting_contacts`; mutations via `can_manage_referral_meetings` + `MeetingLifecyclePolicy`. Write routes: new / edit / schedule / complete / cancel. **Internal attendees (4B.2.3, UAT PASS 2026-08-27).** **External participants (4B.2.4):** add / edit / remove; required name+category+meeting role+attendance; optional professional role/organisation/email/telephone; categories from `MeetingAttendee` allowlist; same-name participants allowed; contacts omitted from Assessor markup; no invitations/emails; completed = final attendance correction only; cancelled/archived deny mutations. Soft completion warning counts all non-final attendees (internal + external). No workflow-stage changes. Phase 4B.2.5 will polish security/accessibility. Meetings remain separate from assessment scheduling.
