...
**Portal UI (4B.2.1–4B.2.3):** Nested routes under `/referrals/{id}/meetings/` (`PortalRouter` rewrite **1.2.5**). View via `can_view_referral_meetings`; contacts via `can_view_referral_meeting_contacts`; mutations via `can_manage_referral_meetings` + `MeetingLifecyclePolicy`. Write routes: new / edit / schedule / complete / cancel. **Internal attendees (4B.2.3, staging UAT PASS 2026-08-27):** add / edit / remove routes; identity immutable after create (wrong user → remove + add); duplicate staff blocked in service (`has_internal_user`; DB index not unique — NON-BLOCKING DOCUMENTED RISK); default attendance `invited`; completed meetings allow final attendance correction only; cancelled + archived deny attendee mutations. Soft completion warning counts non-final (`invited`/`confirmed`) attendees without blocking — manually exercised. Support Worker may be listed as attendee via `VIEW_REFERRALS` eligibility but membership grants no access. External participants remain read-only until 4B.2.4. No emails; no workflow-stage changes. Activation DI wiring defect (null attendee service) discovered and corrected before functional UAT. Deleted-user fallback code-reviewed only. Archived referrals remain mutation-read-only. Meetings remain separate from assessment scheduling.
